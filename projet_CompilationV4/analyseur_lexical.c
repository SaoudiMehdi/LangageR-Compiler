...
void Lire_Mot(){
	...
	*(mot+_compt)='\0';
	...
	}else if(!strcmp(mot,"mean")){
		SYM_COUR.CODE=MEAN_TOKEN;
		strcpy(SYM_COUR.nom,"MEAN_TOKEN");*/
	}else{
		*(mot+_compt) = Car_Cour;  *(mot+ _compt + 1) ='\0'; 
		T_Table_Fonction Fonction = Chercher_Fonction(mot);
		if(Fonction.nombreArgument != -1){
			SYM_COUR.CODE=FONCTION_TOKEN;
			strcpy(SYM_COUR.nom,"FONCTION_TOKEN");
...
T_Table_Fonction Chercher_Fonction(char* nomFonction){
	T_Table_Fonction fonction;
	for(int i=0; i < indiceTableFonction ; i++){
		if(!strcmp(Table_Fonction[i].nom,nomFonction)) return Table_Fonction[i];
	}
	fonction.nombreArgument = -1;
	return fonction;
}
...
int CaractereSigne(char Car_Cour){
	if(Car_Cour=='+'||Car_Cour=='*'||Car_Cour=='-'||Car_Cour=='<'||Car_Cour=='>'||Car_Cour=='/'||Car_Cour==';'||Car_Cour=='%')
		return 1;
	return 0;
}

void Init_Fonctions(){
	add_fonction("mean(",SEQ_TOKEN,2);
	//add_fonction("print(",-1,1);
	add_fonction("class(",-1,1);
	//add_fonction("cat(",-1,1);
	add_fonction("scan(",-1,0);
	add_fonction("max(",VIR_TOKEN,INF);
	add_fonction("min(",VIR_TOKEN,INF);
	add_fonction("list(",VIR_TOKEN,INF);
...
void add_fonction(char* nomFonction, int sep, int nombreArgument){
	strcpy(Table_Fonction[indiceTableFonction].nom, nomFonction);
	Table_Fonction[indiceTableFonction].separateur = sep;
	Table_Fonction[indiceTableFonction].nombreArgument = nombreArgument;
	Table_Fonction[indiceTableFonction].INDICE_TOKEN = EOF_TOKEN + indiceTableFonction + 1 ;
	indiceTableFonction ++;
}