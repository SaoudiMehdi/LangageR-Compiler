...
void AFFEC1(){
	...
	if(SYM_COUR.CODE==AFFOP1_TOKEN){
		int t = CHERCHER_SYM(nom_symbol2);
		...
		int t1 = CHERCHER_SYM(nom_symbol2);
		ECRIRE2();
		int t2 = CHERCHER_SYM(nom_symbol2);
		Test_Symbole(AFFOP1_TOKEN,AFF_ERR);
		Test_Symbole(ID_TOKEN,ID_ERR);
	...
	switch(SYM_COUR.CODE){
		case ID_TOKEN:
			...
			if (r != -1)
			{
				...
				else if(oP == true && r!=2) AJOUTER_SYM(nom_symbol2,r); 
				else{
					compatibiliteError();
				}
			}
		...
		case INT_TOKEN:
			...
			AJOUTER_SYM(nom_symbol2,(CHERCHER_SYM(nom_symbol2)==TFLT)? TFLT : TINT);
			break;
		case FLOAT_TOKEN:
			type_symbole = TFLT;
...
void MIN(){
	...
	switch(SYM_COUR.CODE){
		...
		case FLOAT_TOKEN:
			...
			break;
		case ID_TOKEN:
			AJOUTER_SYM(nom_symbol2,CHERCHER_SYM(nom_symbol));
			Sym_Suiv();
			break;
		case CHAINE_TOKEN:
			AJOUTER_SYM(nom_symbol2,TCHR);
	...
	int typePrec = CHERCHER_SYM(nom_symbol2);
	switch(SYM_COUR.CODE){
		case INT_TOKEN:
		...
		case FLOAT_TOKEN:
			...
			break;
		case ID_TOKEN:
			if(typePrec != 2 && (CHERCHER_SYM(nom_symbol) == 2 || CHERCHER_SYM(nom_symbol) ==1) ) AJOUTER_SYM(nom_symbol2,CHERCHER_SYM(nom_symbol));
			Sym_Suiv();
			break;
		case CHAINE_TOKEN:
			AJOUTER_SYM(nom_symbol2,TCHR);
