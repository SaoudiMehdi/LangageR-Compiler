...
void Lire_Nombre(){
	char nombre_cour[21];
	int i = 0;
	while(i < 20 && ((Car_Cour>='0' && Car_Cour<='9') || Car_Cour=='.')){
	...
	if(CaractereVide(Car_Cour) || CaractereSigne(Car_Cour) || Car_Cour == ',' ||Car_Cour=='\n' || Car_Cour==')' || Car_Cour==':'){
		if(FLOAT){
			...
			SYM_COUR.VAL_INT_FLOAT = atof(nombre_cour);
		}else{
			SYM_COUR.CODE = INT_TOKEN;
			strcpy(SYM_COUR.nom,"INT_TOKEN");
			SYM_COUR.VAL_INT_FLOAT = atoi(nombre_cour);
		}
	}else{
		while(!CaractereVide(Car_Cour))
			Lire_Car();
